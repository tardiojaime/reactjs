...
const Proveedor = () => {
  ...
  const deleteInfo = async (id) => {
    if (id) {
      ...
      if (resul.status === 200) {
        setProveedor((prevRows)=>prevRows.filter((row)=>row.ci !== id));
        setAlert(false);
      }
      else{
        setErrorCar(true);
  ...
  const columnas = useMemo(() => [
    ...
    },
    {
      ...
      align: 'right',
      width: 70,
      valueGetter: (params)=>`${params.row.sexo ==='m' ? 'Masculino' : 'Femenino'}`,
    },
    {
      field: 'edad',
      type: 'number',
  ...
  return (
    <Box m="20px">
      ...
      >
      <Header title="Proveedores" subtitle="Registros de proveedores" />
      <Link to='/proveedor/create'><Fab color="primary" size="medium" aria-label="add"><AddIcon /></Fab></Link>
      </Box>
      <Box
        height="75vh"
        sx={{
